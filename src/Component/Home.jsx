...
function Movies() {
        ...
        let nums = new Array(7).fill(1).map((ele, index) => index + 1);
        console.log(nums);
...
96879b83c44&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${pageNumber
        ...
        return (
                ...
                <>
                        ...
                        <nav aria-label="...">
                                <ul className="pagination pagination-sm d-flex justify-content-center align-items-center column">
                                        {nums.map((pagenumber) => <li onClick={() => getAllTerndings(pagenumber)}
                                                key={pagenumber} className="page-item "><a className={`page-link ${styles.numbers} text-white `} >{pagenumber}</a></li>)}
                                </ul>
                        </nav>
                </>
        )
